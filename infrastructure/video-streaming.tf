# Use the null_resource block to create and publish a new build of our dockerfile every time.
# This is not an ideal solution, and hopefully in the future the docker providers will have a better way of building and publishing images.
...
resource "kubernetes_deployment" "service_deployment" {
  ...
  spec {
    ...
    template {
      ...
      ## Currently, we need to pipe in more information for this microservice to function.
      ## Check out the necessary environment values in the video-streaming folder.
      spec {
        container {
